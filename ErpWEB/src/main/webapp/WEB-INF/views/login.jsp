...
<div class="login-box">
    ...
    <div class="login-box-body">
        ...
        <form   name='loginForm' action="<c:url value='/login' />" id="loginform" method='POST'>
            <div class="form-group has-feedback">
				...
				<div class="form-group has-feedback">
                                           <div id="success2"   style="position: fixed; left: 50%; transform: translate(-50%, 0); display:none">
					                             <img src="/img/processing.gif" height="130px" width="130px">
	                                        </div>

                                          <div id="processing2"     style="position: fixed; left: 50%; transform: translate(-50%, 0); display:none">
			...
			<div class="form-group has-feedback">
                		...
                		<input type="text" name="email"  class="form-control" id="email" placeholder="Login ID" required>
            ...
            <div class="row">
                <div class="col-xs-8">
					<div class="checkbox">
						<input type="checkbox" class="styled" id="1" />
						<label for="1" class="text-default">Remember Me</label>
					</div>
                </div>
                <div class="col-xs-4">
                    <button type="button"  id="btlogin"  onclick="logionValidation()" ng-disabled="loginForm.$invalid"   class="btn btn-info btn-block btn-flat">Sign In</button>
                </div>
              ...
              <div class="row">
              <div id="error2" class="col-xs-12" style="color:red">
	 ...
	 $("#usertype_validation").hide();
	  ...
	  if($("#password").val()!="")
	  {
       ...
       $("#password_validation").hide();
 	     ...
 	     if($("#email").val()!="" && $("#password").val()!="" && $("#usertype").val()!="")
 		  {
 	    	 $("#error2").fadeOut();
 	    	 $("#login_validation").hide();
 	    	 $("#password_validation").hide();
 	    	 
 	    		$.post('/loginValidation?email='+$("#email").val()+'&password='+$("#password").val()).success(function(data) 
 	    	    {
 	    		...
 	    		if(flag==1)
 	    			...
 	    			var email=$("#email").val();
 	    			var url='/userInit/getUserDetailsByUsername?email='+email;
 	    			
 	    		    $.post(url).success(function(userdetail) 
 	    	 	    	    {
 	    				    
 	    			    	$("#email").val(userdetail[0].userName)
 	    	   	           
 	    			    	$("#btlogin").text("Signing in ...")
 	    		...
 	    		if(flag==0)
 	    			{
 	    		     $("#error2").show();
 	 		 	     $("#error2").html("Invalid credentials found !!"); 
 	 		 	     $("#error2").fadeOut(8000);
 	 		 	     $("#success2").fadeOut();
 	    			}
 	    		...
 	    		if(flag==2)
	    			{
	    		     $("#error2").show();
	 		 	     $("#error2").html("Your account is de-activated !!"); 
	 		 	     $("#error2").fadeOut(8000);
	 		 	     $("#success2").fadeOut();
	    			}
